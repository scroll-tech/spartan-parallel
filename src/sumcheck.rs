...
impl ZKSumcheckInstanceProof {
  ...
  /*
  pub fn prove_cubic_with_additive_term_bunched<F>(
    claim: &Scalar,
    blind_claim: &Scalar,
    num_rounds: usize,
    evals_r: &Vec<Scalar>,
    ...
    F: Fn(&Scalar, &Scalar, &Scalar, &Scalar) -> Scalar,
  {
    ...
    for j in 0..num_rounds {
      let (poly, comm_poly) = {
        ...
        for inst in 0..r.len() {
          for i in 0..len {
            let poly_B = poly_B_list[inst];
            let poly_C = poly_C_list[inst];
            let poly_D = poly_D_list[inst];
            // eval 0: bound_func is A(low)
            eval_point_0 += evals_r[inst] * comb_func(&poly_A[i], &poly_B[i], &poly_C[i], &poly_D[i]);
            ...
            let poly_C_bound_point = poly_C[len + i] + poly_C[len + i] - poly_C[i];
            let poly_D_bound_point = poly_D[len + i] + poly_D[len + i] - poly_D[i];
            eval_point_2 += evals_r[inst] * comb_func(
              &poly_A_bound_point,
              &poly_B_bound_point,
              &poly_C_bound_point,
              &poly_D_bound_point,
            ...
            let poly_C_bound_point = poly_C_bound_point + poly_C[len + i] - poly_C[i];
            let poly_D_bound_point = poly_D_bound_point + poly_D[len + i] - poly_D[i];
            eval_point_3 += evals_r[inst] * comb_func(
              &poly_A_bound_point,
              &poly_B_bound_point,
              &poly_C_bound_point,
              &poly_D_bound_point,
