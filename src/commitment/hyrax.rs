...
impl<G: CurveGroup<ScalarField = Scalar>> CommitmentScheme for HyraxScheme<G> {
    ...
    fn commit(poly: &DensePolynomial, gens: &Self::Setup) -> Self::Commitment {
...
impl<G: CurveGroup<ScalarField = Scalar>> HyraxCommitment<G> {
    pub fn commit(
        poly: &DensePolynomial,
...
impl<G: CurveGroup> AppendToTranscript for HyraxCommitment<G> {
    fn append_to_transcript(&self, transcript: &mut Transcript) {
        ...
        for i in 0..self.row_commitments.len() {
            transcript.append_point(b"poly_commitment", &self.row_commitments[i]);
        }
...
impl<G: CurveGroup<ScalarField = Scalar>> HyraxOpeningProof<G> {
    fn protocol_name() -> &'static [u8] {
    ...
    pub fn prove(
        ...
        let (L, _R) = eq.compute_factored_evals_with_l_size(L_size);
    ...
    pub fn verify(
        ...
        let (L, R) = eq.compute_factored_evals_with_l_size(L_size);
...
impl<G: CurveGroup<ScalarField = Scalar>> BatchedHyraxOpeningProof<G> {
    pub fn prove(
        polynomials: &[&DensePolynomial],
        ...
        let rlc_coefficients: Vec<_> = transcript.challenge_vector(b"challenge_vec", polynomials.len());
    ...
    pub fn verify(
        ...
        let rlc_coefficients: Vec<_> = transcript.challenge_vector(b"challenge_vec", openings.len());
